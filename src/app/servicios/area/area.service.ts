...
//  readonly rootUrl = 'http://localhost:8080/cdc/rs';
  ...
  addListaElemento(areaId: Number, listaElemento: listaElemento_Modelo) {
    return this.http.post('/area/registrar/listaElemento/' + areaId, listaElemento, httpOptions);
  }
  ...
  getListaElemento(areaId: Number): Observable<listaElemento_Modelo> {
    return this.http.get<listaElemento_Modelo>('/area/listaElemento/' + areaId);
  }
  updateArea(area: area_Modelo): Observable<respuesta_cdc_Modelo> {
    return this.http.post<respuesta_cdc_Modelo>('/area/editar', area, httpOptions);
  }
  updateListaElemento(areaId: Number, listaElemento: listaElemento_Modelo) {
    return this.http.post('/area/update/listaElemento/' + areaId, listaElemento, httpOptions);
  }
  deleteListaElemento(listaElemento_Id: Number) {
    return this.http.post('/area/delete/listaElemento/' + listaElemento_Id, httpOptions);
  }
}
